...
clretrieveacc --includetaxa=genus,.+ --ngword=environmental,uncultured,unclassified,unidentified,metagenome,metagenomic --taxdb=fungi.taxdb fungi_genus.txt || exit $?
# make BLAST database
cd blastdb || exit $?
...
clretrieveacc --includetaxa=genus,.+ --ngword=environmental,uncultured,unclassified,unidentified,metagenome,metagenomic --acclist=fungi_ITS.txt --taxdb=fungi.taxdb fungi_ITS_genus.txt || exit $?
# extract species-level identified sequences
clretrieveacc --includetaxa=genus,.+,species,.+ --maxrank=species --ngword='species, sp\.$,environmental,uncultured,unclassified,unidentified,metagenome,metagenomic' --acclist=fungi_ITS.txt --taxdb=fungi.taxdb fungi_ITS_species.txt || exit $?
# make BLAST database
cd blastdb || exit $?
# NT-independent, but overall_class-dependent
clextractdupacc --workspace=disk overall_underclass.txt ../fungi_ITS_genus.txt fungi_ITS_genus.txt &
clextractdupacc --workspace=disk overall_underclass.txt ../fungi_ITS_species.txt fungi_ITS_species.txt &
wait
