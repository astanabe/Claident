...
# make table
unless ($tempdbhandle->do("CREATE TABLE accs (id INTEGER NOT NULL PRIMARY KEY, acc TEXT NOT NULL);")) {
	&errorMessage(__LINE__, "Cannot make table.");
}
# prepare SQL statement
my $statement;
unless ($statement = $tempdbhandle->prepare("INSERT INTO accs (id, acc) VALUES (?, ?);")) {
	&errorMessage(__LINE__, "Cannot prepare SQL statement.");
}

print(STDERR "Extracting duplicated accessions...");
{
	...
	foreach my $inputfile (@inputfiles) {
		...
		while (<$inputhandle>) {
			if (/[A-Za-z0-9_]+/) {
				unless ($statement->execute($lineno, $&)) {
...
{
	unless ($statement = $tempdbhandle->prepare("SELECT acc FROM accs GROUP BY acc HAVING COUNT(*) >= " . scalar(@inputfiles))) {
		&errorMessage(__LINE__, "Cannot prepare SQL statement.");
	}
...
Usage
=====
clextractdupacc inputfiles outputfile
