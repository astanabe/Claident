...
# make table
unless ($tempdbhandle->do("CREATE TABLE gis (id INTEGER NOT NULL PRIMARY KEY, gi INTEGER NOT NULL);")) {
	&errorMessage(__LINE__, "Cannot make table.");
}
# prepare SQL statement
my $statement;
unless ($statement = $tempdbhandle->prepare("INSERT INTO gis (id, gi) VALUES (?, ?);")) {
	&errorMessage(__LINE__, "Cannot prepare SQL statement.");
}

print(STDERR "Extracting duplicated GIs...");
{
...
Usage
=====
clextractdupgi inputfiles outputfile
