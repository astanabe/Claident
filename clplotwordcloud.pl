					...
					if ($max) {
						...
						if (-e "$outfileprefix.html" && -d "$outfileprefix\_files") {
							...
							while (!-e "$outfileprefix.png" || (-s "$outfileprefix.png") < 10240) {
								system("timeout 600 $chromeexec --headless --single-process --disable-dev-shm-usage --lang=en --disk-cache-dir=$outfileprefix --user-data-dir=$outfileprefix --crash-dumps-dir=$outfileprefix --screenshot=$outfileprefix.png --window-size=6000,6000 --virtual-time-budget=100000000 $outfileprefix.html 2>> $outfileprefix.log" . ' 1> ' . $devnull);
