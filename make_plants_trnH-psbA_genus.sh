...
clretrieveacc --includetaxa=genus,.+ --ngword=environmental,uncultured,unclassified,unidentified,metagenome,metagenomic --acclist=plants_trnH-psbA.txt --taxdb=plants.taxdb plants_trnH-psbA_genus.txt || exit $?
# extract species-level identified sequences
clretrieveacc --includetaxa=genus,.+,species,.+ --maxrank=species --ngword='species, sp\.$,environmental,uncultured,unclassified,unidentified,metagenome,metagenomic' --acclist=plants_trnH-psbA.txt --taxdb=plants.taxdb plants_trnH-psbA_species.txt || exit $?
# make BLAST database
cd blastdb || exit $?
# NT-independent, but overall_class-dependent
clextractdupacc --workspace=disk overall_underclass.txt ../plants_trnH-psbA_genus.txt plants_trnH-psbA_genus.txt &
clextractdupacc --workspace=disk overall_underclass.txt ../plants_trnH-psbA_species.txt plants_trnH-psbA_species.txt &
wait
