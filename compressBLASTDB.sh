...
cd blastdb
ls overall_class.*.nsq | grep -o -P '^.+\.' | xargs -P `expr $NCPU / 8` -I {} sh -c 'tar -c --use-compress-program="xz -T 8 -9e" -f {}tar.xz {}n*'
ls *_*_genus.nal | grep -o -P '^[^_]+_[^_]+_' | xargs -P `expr $NCPU / 8` -I {} sh -c 'tar -c --use-compress-program="xz -T 8 -9e" -f {}genus.tar.xz {}*.bsl {}*.nal'
