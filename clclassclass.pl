...
sub checkVariables {
	...
	#	&errorMessage(__LINE__, "Two or more input files must be given.");
	#}
	if (-e $outputfolder) {
...
sub makeConcatenatedFiles {
	$filehandleoutput1 = writeFile("concatenated.fastq.gz");
	$filehandleoutput2 = writeFile("concatenated.contigmembers.gz");
	foreach my $inputfile (@inputfiles) {
		...
		while (<$filehandleinput1>) {
			if ($tempnline % 4 == 1 && /^\@/) {
				s/^(\@dereplicated_\d+)(\r?\n?)$/$1\__$filename$2/;
				s/^(\@withinsampleotu_\d+)(\r?\n?)$/$1\__$filename$2/;
				s/^\@(amongsampleotu_\d+__\S+\r?\n?)$/\@past$1/;
			}
			print($filehandleoutput1 $_);
			$tempnline ++;
		}
		close($filehandleinput1);
		my $contigmembers = $inputfile;
		$contigmembers =~ s/\.fastq\.gz$/.contigmembers.gz/;
		$contigmembers =~ s/\.fastq$/.contigmembers.gz/;
		$filehandleinput1 = &readFile("$root/$contigmembers");
		while (<$filehandleinput1>) {
			s/^(dereplicated_\d+)\t/$1\__$filename\t/;
			s/^(withinsampleotu_\d+)\t/$1\__$filename\t/;
			s/^(amongsampleotu_\d+__\S+\t)/past$1/;
...
sub runAssams {
	...
	if ($nsteps == 2) {
		# run first assams
		if (system("assams$assams1option --numthreads=$numthreads concatenated.fastq.gz firstrun.bnk 1> $devnull")) {
			&errorMessage(__LINE__, "Cannot run \"assams$assams1option --numthreads=$numthreads concatenated.fastq.gz firstrun.bnk\".");
		}
		# run second assams
		...
		if (system("assams$assams2option --numthreads=$numthreads concatenated.fastq.gz assembled.bnk 1> $devnull")) {
			&errorMessage(__LINE__, "Cannot run \"assams$assams2option --numthreads=$numthreads concatenated.fastq.gz assembled.bnk\".");
		}
	}
...
Usage
=====
clclassclass options inputfiles outputfolder
